...
export const ImageUploader: ChakraComponent<'div', ImageUploaderProps> = ({
  ...
  const isPending = useReactiveVar(imageUploadPending)
  ...
  return (
    ...
    >
      ...
      {images.length === 1 && (
        <Center h="100%" border="4px solid rgba(0, 0, 0, 0)" overflow="hidden">
          <ImgPreview src={images[0]} alt="uploaded_image" isMultiple={false} />
        </Center>
      )}
      {images.length > 1 && (
